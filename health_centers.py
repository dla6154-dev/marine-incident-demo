...
API_URL = "https://safemap.go.kr/openapi2/IF_0024"
...
def parse_center(item: dict) -> HealthCenter | None:
    try:
    ...
    return HealthCenter(
        ...
        type=clean_text(item.get("dutydivname")) or "보건소",
        lat=round(lat, 6),
        lng=round(lng, 6),
        x=x,
        y=y,
...
def extract_items(payload: dict) -> tuple[list[dict], int]:
    ...
    if header.get("resultCode") != "00":
        raise RuntimeError(header.get("resultMsg") or "health center API request failed")
...
def fetch_health_centers(
    ...
    *,
    ...
    while True:
        ...
        for item in items:
            center = parse_center(item)
            if center:
                centers.append(asdict(center))

        if len(centers) >= total_count or not items:
            break
        page_no += 1

    if not centers:
        raise RuntimeError("usable health center coordinates were not found in the API response")
